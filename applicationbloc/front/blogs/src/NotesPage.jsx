...
export default function NotesPage({ token, user, onLogout }) {
  ...
  const [mobileTab, setMobileTab]   = useState("notes"); // "notes" | "stats"
  ...
  const exportNote = (note) => {
    ...
    const priorityColor = { haute:[192,57,43], moyenne:[140,107,191], basse:[26,158,143] };
    ...
    script.onload = () => {
      ...
      let y = 20;
      const [r,g,b] = priorityColor[note.priority] || priorityColor.basse;
      doc.setFillColor(r,g,b);
      doc.rect(0, 0, W, 3, "F");
      doc.setFontSize(9); doc.setTextColor(138,147,168);
      doc.text("NoteFlow", margin, y);
      const dateStr = new Date(note.created_at).toLocaleDateString("fr-FR", { day:"numeric", month:"long", year:"numeric" });
      doc.text(dateStr, W - margin, y, { align:"right" });
      y += 10;
      doc.setFontSize(10); doc.setTextColor(r,g,b); doc.setDrawColor(r,g,b);
      const badgeLabel = priorityLabel[note.priority] || note.priority;
      const badgeW = doc.getTextWidth(badgeLabel) + 8;
      doc.roundedRect(margin, y - 4, badgeW, 7, 2, 2, "S");
      doc.text(badgeLabel, margin + 4, y + 1.5);
      y += 14;
      doc.setFontSize(22); doc.setTextColor(33,46,83); doc.setFont("helvetica","bold");
      const titleLines = doc.splitTextToSize(note.title, maxW);
      doc.text(titleLines, margin, y);
      y += titleLines.length * 9 + 4;
      doc.setDrawColor(226,230,239); doc.setLineWidth(0.4);
      doc.line(margin, y, W - margin, y); y += 10;
      if (note.content && note.content.trim()) {
        doc.setFontSize(12); doc.setFont("helvetica","normal"); doc.setTextColor(90,100,128);
        ...
        doc.setFontSize(12); doc.setFont("helvetica","italic"); doc.setTextColor(176,184,204);
        doc.text("Aucun contenu.", margin, y);
      }
      const filename = note.title.slice(0,40).replace(/[^a-z0-9]/gi,"-").toLowerCase();
  ...
  /* ── Mobile Form Drawer ── */
  const FormDrawer = () => (
    <div style={isMobile ? s.drawerOverlay : {}}>
      {isMobile && <div style={s.drawerBackdrop} onClick={resetForm}/>}
      <div style={isMobile ? s.drawer : s.formCard}>
        ...
        <div style={isMobile ? s.formBodyMobile : s.formBody}>
          <div style={s.formCol1}>
            <div style={s.fField}>
              ...
              {titleEmpty && <p style={s.titleHint}>⚠ Le titre est obligatoire pour enregistrer.</p>}
            </div>
            <div style={s.fField}>
              <label style={s.flabel}>Contenu <span style={s.optional}>(optionnel)</span></label>
              <textarea style={{ ...fi("content"), minHeight: isMobile ? 90 : 110, resize:"vertical", padding:"12px 14px", lineHeight:1.7 }}
          ...
          <div style={isMobile ? s.formCol2Mobile : s.formCol2}>
            <label style={s.flabel}>Niveau de priorité</label>
            <div style={isMobile ? s.priorityGroupRow : s.priorityGroup}>
              {["haute","moyenne","basse"].map(p => {
                const pc = PRIORITY[p];
                const active = form.priority === p;
                return (
                  <button key={p} onClick={() => setForm({...form, priority:p})}
                    style={{ ...s.priorityBtn, ...(isMobile ? s.priorityBtnMobile : {}), ...(active ? { background:pc.bg, border:`2px solid ${pc.color}`, color:pc.text } : {}) }}>
            ...
            <div style={s.formActions}>
              <button
                style={{ ...s.btnSave, ...(titleEmpty ? s.btnSaveDisabled : {}) }}
                onClick={handleSave}
                disabled={titleEmpty}
                title={titleEmpty ? "Le titre est obligatoire" : ""}
              >
  ...
  return (
    <div style={{ ...s.bg, paddingBottom: isMobile ? 72 : 0 }}>
      ...
      <header style={isMobile ? s.headerMobile : s.header}>
        <div style={s.hLeft}>
          ...
          <div>
            ...
            {!isMobile && <p style={s.hWelcome}>Bonjour</p>}
          </div>
        </div>
        <div style={s.hRight}>
          {!isMobile && (
      ...
      <main style={isMobile ? s.mainMobile : s.main}>
        ...
        {/* Desktop stats always visible; mobile: only on stats tab */}
        {(!isMobile || mobileTab === "stats") && <StatsGrid />}

        {/* Desktop form */}
        {!isMobile && showForm && <FormDrawer />}
          ...
          <>
            ...
            <div style={s.mobileFilterRow}>
              {[
                ...
                <button key={f.k}
                  style={{ ...s.fBtn, ...(filter===f.k ? s.fBtnActive : {}), fontSize:12, padding:"6px 12px" }}
                  onClick={() => setFilter(f.k)}>
                  {f.label}
                </button>
              ))}
        ...
        {/* Notes grid — shown on desktop always, on mobile only on "notes" tab */}
        {(!isMobile || mobileTab === "notes") && (
          loading ? (
                  ...
                  <div key={note.id} style={{ ...s.noteCard, borderTopColor:pc.color }}>
                    ...
                    <div style={s.noteInner}>
                      ...
                      <div style={s.noteFoot}>
                        ...
                        <button style={s.btnPdf} onClick={() => exportNote(note)}>
                          <svg width="12" height="12" fill="none" viewBox="0 0 12 12"><path d="M2 1h5l3 3v7a1 1 0 01-1 1H2a1 1 0 01-1-1V2a1 1 0 011-1z" stroke="#7C5CBF" strokeWidth="1.2" strokeLinejoin="round"/><path d="M7 1v3h3M3 6.5h6M3 8.5h4" stroke="#7C5CBF" strokeWidth="1.2" strokeLinecap="round"/></svg>
                          PDF
                        </button>
      ...
      {isMobile && (
        <nav style={s.bottomNav}>
          <button style={{ ...s.bottomNavBtn, ...(mobileTab==="notes" ? s.bottomNavBtnActive : {}) }}
            onClick={() => setMobileTab("notes")}>
            <svg width="20" height="20" fill="none" viewBox="0 0 22 22">
              <rect x="3" y="3" width="14" height="16" rx="2" stroke="currentColor" strokeWidth="1.6"/>
          ...
          <button style={s.bottomNavFab} onClick={() => { resetForm(); setShowForm(true); }}>
            <svg width="22" height="22" fill="none" viewBox="0 0 22 22"><path d="M11 3v16M3 11h16" stroke="white" strokeWidth="2.2" strokeLinecap="round"/></svg>
          </button>

          <button style={{ ...s.bottomNavBtn, ...(mobileTab==="stats" ? s.bottomNavBtnActive : {}) }}
            ...
            <span style={s.bottomNavLabel}>Stats</span>
          </button>
        </nav>
      )}
...
const s = {
  ...
  headerMobile: { background:C.navy, padding:"0 16px", display:"flex", justifyContent:"space-between", alignItems:"center", height:56, position:"sticky", top:0, zIndex:100, boxShadow:"0 2px 20px rgba(33,46,83,0.2)" },
  hLeft: { display:"flex", alignItems:"center", gap:14 },
  hLogo: { width:38, height:38, background:"rgba(255,255,255,0.1)", borderRadius:10, border:"1px solid rgba(255,255,255,0.15)", display:"flex", alignItems:"center", justifyContent:"center" },
  ...
  mainMobile: { padding:"16px 12px", maxWidth:"100%" },
  ...
  statIconWrap: { fontSize:18, minWidth:32, height:32, borderRadius:10, background:"rgba(255,255,255,0.6)", display:"flex", alignItems:"center", justifyContent:"center" },
  statNum:   { fontSize:24, fontFamily:font.display, lineHeight:1 },
  ...
  formBodyMobile: { display:"flex", flexDirection:"column", gap:16 },
  formCol1: {},
  formCol2: { display:"flex", flexDirection:"column", gap:14 },
  formCol2Mobile: { display:"flex", flexDirection:"column", gap:12 },
  ...
  finput: { width:"100%", padding:"11px 14px", background:C.offwhite, border:`1.5px solid ${C.border}`, borderRadius:10, fontSize:14, color:C.navy, outline:"none", transition:"all 0.2s", fontFamily:font.body, boxSizing:"border-box" },
  ...
  priorityGroupRow: { display:"flex", flexDirection:"row", gap:8 },
  ...
  priorityBtnMobile: { flex:1, padding:"10px 8px", fontSize:12, justifyContent:"center" },
  ...
  btnSaveDisabled: { background:"#C8D0DB", boxShadow:"none", cursor:"not-allowed", opacity:0.7 },
  btnCancelForm: { padding:"10px", background:"#EEF0F5", color:C.textMuted, border:"none", borderRadius:10, fontSize:14, cursor:"pointer" },
  titleHint: { fontSize:12, color:C.terra, marginTop:5, fontWeight:500 },

  /* ── Mobile drawer ── */
  drawerOverlay: { position:"fixed", inset:0, zIndex:500, display:"flex", alignItems:"flex-end" },
  drawerBackdrop: { position:"absolute", inset:0, background:"rgba(33,46,83,0.45)", backdropFilter:"blur(4px)" },
  drawer: { position:"relative", zIndex:1, width:"100%", background:C.white, borderRadius:"20px 20px 0 0", padding:"12px 16px 32px", maxHeight:"90vh", overflowY:"auto", boxShadow:"0 -8px 40px rgba(33,46,83,0.18)", animation:"slideUp 0.3s ease" },
  drawerHandle: { width:40, height:4, background:C.border, borderRadius:4, margin:"0 auto 16px", display:"block" },
  ...
  mobileFilterRow: { display:"flex", gap:8, flexWrap:"nowrap", overflowX:"auto", paddingBottom:10, marginBottom:12, scrollbarWidth:"none" },
  ...
  btnPdf:  { display:"flex", alignItems:"center", gap:5, padding:"6px 12px", background:"#F0EBF9", color:"#7C5CBF", border:"1px solid #D9CEED", borderRadius:8, cursor:"pointer", fontSize:12, fontWeight:500 },
  ...
  /* ── Mobile bottom nav ── */
  bottomNav: { position:"fixed", bottom:0, left:0, right:0, height:64, background:C.white, borderTop:`1px solid ${C.border}`, display:"flex", alignItems:"center", justifyContent:"space-around", zIndex:200, boxShadow:"0 -4px 20px rgba(33,46,83,0.08)" },
  bottomNavBtn: { display:"flex", flexDirection:"column", alignItems:"center", gap:4, padding:"8px 24px", background:"none", border:"none", cursor:"pointer", color:C.textMuted },
  bottomNavBtnActive: { color:C.teal },
  bottomNavLabel: { fontSize:11, fontWeight:500 },
  bottomNavFab: { width:52, height:52, borderRadius:"50%", background:`linear-gradient(135deg,${C.teal},${C.tealDark})`, border:"none", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", boxShadow:`0 4px 16px ${C.teal}60`, marginBottom:8 },
};