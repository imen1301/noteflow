    ...
    return (
      <div style={s.mobileBg}>
        ...
        <div style={s.mobileHeader}>
          ...
          <div style={s.mobileHeaderText}>
            <p style={s.mobileEyebrow}>Nouveau membre</p>
            <h1 style={s.mobileHeroH1}>Rejoignez <em style={{color:C.sage, fontStyle:"normal"}}>NoteFlow</em></h1>
          </div>
        </div>

        {/* Mobile form card */}
        <div style={s.mobileFormCard}>
          <div style={s.mobileFormTop}>
            <p style={s.formEyebrow}>Inscription</p>
            <h2 style={s.mobileFormTitle}>Créer un compte</h2>
        ...
        {/* Mobile steps strip */}
        <div style={s.mobileStepsStrip}>
          {[
            { num:"01", title:"Créer un compte" },
            { num:"02", title:"Se connecter" },
            { num:"03", title:"Gérer vos notes" },
          ].map((step, i) => (
            <div key={i} style={s.mobileStepItem}>
              <div style={s.mobileStepNum}>{step.num}</div>
              <p style={s.mobileStepTitle}>{step.title}</p>
            </div>
          ))}
        </div>
      </div>
...
const s = {
  ...
  shape2: { position:"absolute", width:350, height:350, borderRadius:"50%", background:`radial-gradient(circle, ${C.sageLight} 0%, transparent 65%)`, bottom:-80, left:"30%", pointerEvents:"none" },
  ...
  /* ── Mobile ── */
  mobileBg: { minHeight:"100vh", background:C.offwhite, fontFamily:font.body, display:"flex", flexDirection:"column" },

  mobileHeader: { background:`linear-gradient(155deg, ${C.navy} 0%, #1A2440 100%)`, padding:"28px 20px 32px", position:"relative", overflow:"hidden" },
  mobileBrand: { display:"flex", alignItems:"center", gap:10, marginBottom:20 },
  mobileBrandIcon: { width:36, height:36, borderRadius:10, background:`${C.teal}30`, border:`1px solid rgba(255,255,255,0.14)`, display:"flex", alignItems:"center", justifyContent:"center" },
  mobileBrandName: { color:C.white, fontSize:18, fontWeight:600 },
  mobileHeaderText: {},
  mobileEyebrow: { fontSize:11, letterSpacing:"2px", textTransform:"uppercase", color:C.blush, fontWeight:500, marginBottom:8 },
  mobileHeroH1: { fontFamily:font.display, fontSize:30, color:C.white, lineHeight:1.25, fontStyle:"normal" },

  mobileFormCard: { flex:1, background:C.white, borderRadius:"20px 20px 0 0", marginTop:-16, padding:"28px 20px 40px", boxShadow:"0 -4px 24px rgba(33,46,83,0.1)" },
  mobileFormTop: { marginBottom:24 },
  mobileFormTitle: { fontFamily:font.display, fontSize:26, color:C.navy, lineHeight:1.2, marginBottom:6 },

  mobileStepsStrip: { display:"flex", justifyContent:"space-around", background:C.navy, padding:"16px 12px", gap:8 },
  mobileStepItem: { display:"flex", flexDirection:"column", alignItems:"center", gap:6, flex:1 },
  mobileStepNum: { width:30, height:30, borderRadius:8, background:`${C.teal}30`, border:`1px solid ${C.teal}60`, display:"flex", alignItems:"center", justifyContent:"center", color:C.blush, fontSize:11, fontWeight:700 },
  mobileStepTitle: { color:"rgba(255,255,255,0.75)", fontSize:10, fontWeight:500, textAlign:"center", lineHeight:1.4 },
  ...
  inputBox: { display:"flex", alignItems:"center", gap:10, background:C.offwhite, border:`1.5px solid ${C.border}`, borderRadius:12, padding:"0 16px", height:52, transition:"all 0.2s" },
  inputFocus: { borderColor:C.teal, boxShadow:`0 0 0 4px ${C.tealLight}80`, background:C.white },
